...
unsigned int nStakeMaxAge = 2 * 30 * 24 * 60 * 60;  // 2 months
unsigned int nModifierInterval = 10 * 60;           // time to elapse before new modifier is computed
unsigned int nTargetTimespan = 10 * 60;
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    if (pindexLast->nHeight < 1001) {
        return bnTargetLimit.GetCompact(); // set diff to bnTargetLimit to generate coinbase
    }
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        ...
        uint64_t nCoinAge;
        int64_t nCoinValue;
        if (!vtx[1].GetCoinAge(txdb, nCoinAge, nCoinValue))
            return error("ConnectBlock() : %s unable to get coin age for coinstake", vtx[1].GetHash().ToString().substr(0,10).c_str());

        int64_t nCalculatedStakeReward = GetProofOfStakeReward(nCoinAge, nCoinValue);
...
bool CTransaction::GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge, int64_t& nCoinValue) const
{
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "MMOCoin 2018";
        CTransaction txNew;
        txNew.nTime = 1519171200;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1519171200;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 87389;

        assert(block.hashMerkleRoot == uint256("0xe2a49c9b4be4b1bca1d1c22aaf86406f8a5efe3335707426b2731f97ed9c777b"));
...
unsigned char pchMessageStart[4] = { 0x61, 0x19, 0x91, 0x16 };
