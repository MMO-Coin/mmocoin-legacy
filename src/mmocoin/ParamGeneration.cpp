...
///
/// \brief      Parameter manipulation routines for the MMOCoin cryptographic
///             components.
///
...
namespace libmmocoin {

/// \brief Fill in a set of MMOCoin parameters from a modulus "N".
...
///
/// \throws         MMOCoinException if the process fails
///
...
/// be known to any party, or the security of MMOCoin is
...
CalculateParams(Params &params, Bignum N, string aux, uint32_t securityLevel)
{
	...
	if (NLen < 1023) {
		throw MMOCoinException("Modulus must be at least 1023 bits");
	}

	// Verify that "securityLevel" is  at least 80 bits (minimum).
	if (securityLevel < 80) {
		throw MMOCoinException("Security level must be at least 80 bits.");
	}
	...
	*pLen = *qLen = 0;
	...
	if (securityLevel < 80) {
		...
		*pLen = 3072;
		...
		throw MMOCoinException("Security level not supported.");
	}

	if (*pLen > maxPLen) {
		throw MMOCoinException("Modulus size is too small for this security level.");
	}
}
...
deriveIntegerGroupParams(uint256 seed, uint32_t pLen, uint32_t qLen)
{
	...
	if ((uint32_t)(result.modulus.bitSize()) < pLen ||          // modulus is pLen bits long
		...
		throw MMOCoinException("Group parameters are not valid");
	}
...
deriveIntegerGroupFromOrder(Bignum &groupOrder)
{
	...
	throw MMOCoinException("Too many attempts to generate Schnorr group.");
}
                              ...
                              uint256 *resultPseed, uint256 *resultQseed)
{
	...
	if (qLen > (sizeof(seed)) * 8) {
		...
		// throw MMOCoinException("Seed is too short to support the required security level.");
	}
	...
	*resultGroupOrder = generateRandomPrime(qLen, seed, &qseed, &qgen_counter);
	...
	// Throw an exception.
	throw MMOCoinException("Unable to generate a prime modulus for the group");
}
...
/// \throws                             A MMOCoinException if error.
///
...
calculateGroupGenerator(uint256 seed, uint256 pSeed, uint256 qSeed, Bignum modulus, Bignum groupOrder, uint32_t index)
{
	...
	if (index > 255) {
		throw MMOCoinException("Invalid index for group generation");
	}
	...
	throw MMOCoinException("Unable to find a generator, too many attempts");
}
                    ...
                    uint32_t *prime_gen_counter)
{
	...
	if (primeBitLen < 2) {
		throw MMOCoinException("Prime length is too short");
	}
	...
	if (primeBitLen < 33) {
		...
		throw MMOCoinException("Unable to find prime in Shawe-Taylor algorithm");
	...
	throw MMOCoinException("Unable to generate random prime (too many tests)");
}
