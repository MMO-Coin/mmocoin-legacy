...
* @file       Tests.cpp
*
* @brief      Test routines for MMOCoin.
*
...
Test_GenerateGroupParams()
{
	...
	for (count = 0; count < 1; count++) {
		...
		try {
			group = deriveIntegerGroupParams(calculateSeed(GetTestModulus(), "test", MMOCOIN_DEFAULT_SECURITYLEVEL, "TEST GROUP"), pLen, qLen);
...
Test_ParamGen()
{
	...
	try {
		...
		Params testParams(GetTestModulus(),MMOCOIN_DEFAULT_SECURITYLEVEL);
...
Test_RunAllTests()
{
	...
	// Run through all of the MMOCoin tests
